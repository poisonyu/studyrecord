...
type Timer interface {
	...
	// 通过函数的方法添加任务
	// cronName:cornName  spec:时间格式，时间规则   task:任务函数，触发时调用的函数   taskName:任务名称
...
func (t *timer) AddTaskByFunc(cronName string, spec string, fun func(), taskName string, option ...cron.Option) (cron.EntryID, error) {
	...
	if _, ok := t.cronList[cronName]; !ok {
		...
		t.cronList[cronName] = &taskManager{
			corn:  cron.New(option...), // 创建cron对象，用于管理定时任务
			tasks: tasks,
		}
	}
	id, err := t.cronList[cronName].corn.AddFunc(spec, fun) // 向管理器中添加定时任务
